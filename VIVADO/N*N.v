...
module param_ai_accelerator #(
    parameter N = 8,           // Scalable Parameter! Set to 256 for a massive array
    parameter DATA_W = 8,
    parameter PSUM_W = 16
) (
    input  wire clk,
        ...
        for (i = 0; i < N; i = i + 1) begin : skew_row
            if (i == 0) begin
                ...
                always @(posedge clk) begin
                    if (reset) begin
                        for (integer d = 0; d < i; d = d + 1)
                            delay_regs[d] <= 0;
                    end else begin
                        delay_regs[0] <= raw_in_pixels_left[i*DATA_W +: DATA_W];
                        for (integer d = 1; d < i; d = d + 1)
    ...
    generate
        for (j = 0; j < N; j = j + 1) begin : unskew_col
            ...
            if (delay_len == 0) begin
                ...
                always @(posedge clk) begin
                    if (reset) begin
                        for (integer d = 0; d < delay_len; d = d + 1)
                            delay_regs[d] <= 0;
                    end else begin
                        delay_regs[0] <= core_out_psums_bottom[j*PSUM_W +: PSUM_W];
                        for (integer d = 1; d < delay_len; d = d + 1)
                            delay_regs[d] <= delay_regs[d-1];
                    end
                end
...
module param_array_core #(
    ...
    // Wires between PEs
    // pixel_wires[i][j] is the output of PE(i,j) going to PE(i, j+1)
    // psum_wires[i][j] is the output of PE(i,j) going to PE(i+1, j)
    ...
    generate
        for (r = 0; r < N; r = r + 1) begin : row_gen
            for (c = 0; c < N; c = c + 1) begin : col_gen
                ...
                // Pixel input (from left or from adjacent PE)
                if (c == 0) begin
                ...
                // PSum input (from top or from adjacent PE)
                if (r == 0) begin
                ...
                // Assign to outputs at the boundaries
                if (c == N - 1) begin
...
// ============================================================================
// MODULE 3: pe (The Single Processing Element - Stays Exactly the Same!)
// ============================================================================
module pe #(
    parameter DATA_W = 8,
    parameter PSUM_W = 16
    ...
    input  wire [DATA_W-1:0] in_pixel,     // Pixel input from left
    input  wire [DATA_W-1:0] in_weight,    // Weight to load
    input  wire [PSUM_W-1:0] in_psum,      // Partial sum from top
    
    output reg  [DATA_W-1:0] out_pixel,    // Pipelined Pixel output to right
    output reg  [PSUM_W-1:0] out_psum      // Pipelined Partial sum output to bottom
);

    // Internal Stationary Weight Register
